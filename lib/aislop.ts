...
export const PROGRESS = [
  ...
  ["та я тут, просто довго", "ще тримаю, не переклацуй"],
] as const;
...
export async function askOrchestrator(opts: {
  ...
  try {
    const response = await fetch(`${cfg.base}/chat-messages`, {
      ...
      // Було 120 с, і цього не хватало: гілка запуску Дня 3 стукає у вебхук
      // ДО ноди персони, тому на таймауті людина читала «не вийшло» вже після
      // того, як стаття почала писатися — і тиснула команду ще раз. Один такий
      // випадок дав три статті за одним планом. Discord тримає токен взаємодії
      // 15 хвилин, тож 240 с — це запас, а не межа.
